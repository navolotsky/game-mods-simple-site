...
DEFAULT_MIN_NUM_ROWS_TO_GEN = 1
DEFAULT_MAX_NUM_ROWS_TO_GEN = 100
...
def main(*, test_data_dir=DEFAULT_TEST_DATA_DIR, seed=DEFAULT_SEED, batch_size=DEFAULT_BATCH_SIZE,
         min_num_rows_to_gen=DEFAULT_MIN_NUM_ROWS_TO_GEN, max_num_rows_to_gen=DEFAULT_MAX_NUM_ROWS_TO_GEN,
         clear_tables=DEFAULT_CLEAR_TABLES):
    ...
    min_num = min_num_rows_to_gen
    max_num = max_num_rows_to_gen
    ...
    users = []
    user_profiles = []
    for i in range(randint(min_num, max_num)):
        ...
        user_profiles.append(m.UserProfile(user=user, avatar=None if not test_avatars else choice(test_avatars),
                                           description="" if random_true() else f"description of user {username}"))
    ...
    for i in range(NUM_GAMES):
        games.append(m.Game(name=(name := gen_random_string("game")),
                            description="" if random_true() else f"description of game {name}"))
    ...
    for i in range(NUM_MOD_CATEGORIES):
        mod_categories.append(m.ModCategory(name=(name := gen_random_string("category")),
                                            description="" if random_true() else f"description of category {name}"))
    m.ModCategory.objects.bulk_create(mod_categories, batch_size)

    # Generate models.Mod
    mods = []
    for _ in range(randint(min_num, max_num)):
        mods.append(m.Mod(game=choice(games), author=choice(users), hidden=random_true()))
    ...
    for mod in mods:
        ...
        for i in range(randint(MIN_NUM_MOD_VERSIONS, MAX_NUM_MOD_VERSIONS)):
            ...
            mod_versions.append(
                m.ModVersion(mod=mod, title=f"title of {prefix}mod_version{i + 1} of mod_{mod.pk}",
                             short_description="" if random_true() else f"short_description of mod_version{i + 1}",
                             full_description="" if random_true() else f"full_description of mod_version{i + 1}",
                             number=gen_version_num() if not mod_versions else random_bump(mod_versions[-1].number),
                             comment="" if random_true() else f"comment of {prefix}mod_version{i + 1}",
                             hidden=hidden))
        if not mod_versions:
        ...
        for mod_version in mod_versions:
            ...
            for i in range(randint(MIN_NUM_MOD_VERSION_DOWNLOAD_LINKS, MAX_NUM_MOD_VERSION_DOWNLOAD_LINKS)):
                mod_version_downloads_links.append(
                    m.ModDownloadLink(mod_version=mod_version,
                                      ...
                                      comment="" if random_true() else f"comment of download_link{i + 1}"))
            ...
            mod_version_has_own_images = random_true() or not prev_mod_version_images
            ...
            for image in prev_mod_version_images:
                delete_image = random_true()
                if delete_image:
                    continue
                replace_image = random_true()
                if replace_image:
            ...
            mod_version.main_image = (
                None if random_true() or not last_mod_version_images else choice(last_mod_version_images))
            prev_mod_version_images = last_mod_version_images

        m.ModVersion.objects.bulk_update(mod_versions, ("main_image",), batch_size)
        mod.showed_version = None if random_true() else choice(mod_versions)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--min_num_rows_to_gen", type=int, default=DEFAULT_MIN_NUM_ROWS_TO_GEN,
                        help="first argument passed to random.randint() which used to choose how many rows "
                             "to generate per some tables (default: %(default)s)")
    parser.add_argument("--max_num_rows_to_gen", type=int, default=DEFAULT_MAX_NUM_ROWS_TO_GEN,
